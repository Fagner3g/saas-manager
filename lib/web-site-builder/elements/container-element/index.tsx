      ...
      return (
        <ElementActions element={element} key={element.id}>
          <div
            ...
            onDragEnd={onDragEnd}
            onDrop={onDrop}
            className={cn("m-2 grid grid-cols-2 border p-2", { "border border-green-500": over })}
          ></div>
        </ElementActions>
      );
    }
    ...
    return (
      <ElementActions element={element} key={element.id}>
        <div
          ...
          onDragEnd={onDragEnd}
          onDrop={onDrop}
          className={cn("m-2 grid grid-cols-2 border p-2", { "border border-green-500": over }, className)}
        >
          {element.content.map((el) => {
            const PreviewComponent = el.previewComponent;
            return <PreviewComponent key={el.id} instance={el} />;
          })}
        </div>
      </ElementActions>
    );
